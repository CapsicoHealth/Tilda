   ...
   protected final void getUpsertQueryPart(Connection C, StringBuilder str) throws Exception
     {
       ...
       switch (__LookupId)
        {
          case 1:
                TILDA__FORMANSWER_Factory.COLS.FORMREFNUM.getShortColumnVarForSelect(C, str);
                str.append(", ");
                TILDA__FORMANSWER_Factory.COLS.FIELD.getShortColumnVarForSelect(C, str);
             break;
       ...
       boolean first = true;
       str.append("set ");
       if (__Changes.intersects(TILDA__FORMANSWER_Factory.COLS.FORMREFNUM._Mask ) == true) { if (first == true) first = false; else str.append("    ,"); TILDA__FORMANSWER_Factory.COLS.FORMREFNUM.getShortColumnVarForSelect(C, str) ; str.append("=EXCLUDED."); TILDA__FORMANSWER_Factory.COLS.FORMREFNUM.getShortColumnVarForSelect(C, str) ; str.append("\n"); }
       if (__Changes.intersects(TILDA__FORMANSWER_Factory.COLS.FIELD._Mask      ) == true) { if (first == true) first = false; else str.append("    ,"); TILDA__FORMANSWER_Factory.COLS.FIELD.getShortColumnVarForSelect(C, str)      ; str.append("=EXCLUDED."); TILDA__FORMANSWER_Factory.COLS.FIELD.getShortColumnVarForSelect(C, str)      ; str.append("\n"); }
       if (__Changes.intersects(TILDA__FORMANSWER_Factory.COLS.VALUE._Mask      ) == true) { if (first == true) first = false; else str.append("    ,"); TILDA__FORMANSWER_Factory.COLS.VALUE.getShortColumnVarForSelect(C, str)      ; str.append("=EXCLUDED."); TILDA__FORMANSWER_Factory.COLS.VALUE.getShortColumnVarForSelect(C, str)      ; str.append("\n"); }
       if (__Changes.intersects(TILDA__FORMANSWER_Factory.COLS.LASTUPDATED._Mask) == true) { if (first == true) first = false; else str.append("    ,"); TILDA__FORMANSWER_Factory.COLS.LASTUPDATED.getShortColumnVarForSelect(C, str); str.append("=EXCLUDED."); TILDA__FORMANSWER_Factory.COLS.LASTUPDATED.getShortColumnVarForSelect(C, str); str.append("\n"); }
       if (__Changes.intersects(TILDA__FORMANSWER_Factory.COLS.DELETED._Mask    ) == true) { if (first == true) first = false; else str.append("    ,"); TILDA__FORMANSWER_Factory.COLS.DELETED.getShortColumnVarForSelect(C, str)    ; str.append("=EXCLUDED."); TILDA__FORMANSWER_Factory.COLS.DELETED.getShortColumnVarForSelect(C, str)    ; str.append("\n"); }
       str.append("returning "); TILDA__FORMANSWER_Factory.COLS.REFNUM.getShortColumnVarForSelect(C, str);
     }
     ...
     __Init = InitMode.LOOKUP;
      ...
      __Saved_field       = _field       = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) { __Nulls.or(TILDA__FORMANSWER_Factory.COLS.FIELD._Mask      ); _field = null; }
                            _value       = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) { __Nulls.or(TILDA__FORMANSWER_Factory.COLS.VALUE._Mask      ); _value = null; }
                                                    _created     = DateTimeUtil.toZonedDateTime(RS.getTimestamp(++i), OCCLocalZone); if (RS.wasNull() == true) { __Nulls.or(TILDA__FORMANSWER_Factory.COLS.CREATED._Mask    ); _created = null; }
                                                    _lastUpdated = DateTimeUtil.toZonedDateTime(RS.getTimestamp(++i), OCCLocalZone); if (RS.wasNull() == true) { __Nulls.or(TILDA__FORMANSWER_Factory.COLS.LASTUPDATED._Mask); _lastUpdated = null; }
                                                    _deleted     = DateTimeUtil.toZonedDateTime(RS.getTimestamp(++i), OCCLocalZone); if (RS.wasNull() == true) { __Nulls.or(TILDA__FORMANSWER_Factory.COLS.DELETED._Mask    ); _deleted = null; }
   ...
   public String getCSVHeader(String exportName) throws Exception
    {
      switch (exportName)
        { 
          case "": return tilda.tutorial.data.FormAnswer_Factory.getCSVHeader();
