...
This Table contains the following columns:<BLOCKQUOTE>
 <TABLE id="Measure_TBL" border="0px" cellpadding="3px" cellspacing="0px" style="border:1px solid #BBB;">
...
public abstract class TILDA__MEASURE implements tilda.interfaces.WriterObject, tilda.interfaces.OCCObject, tilda.interfaces.JSONable, tilda.interfaces.CSVable
 {
   protected transient static final Logger LOG = LogManager.getLogger(TILDA__MEASURE.class.getName());

   public transient static final Class<TILDA__MEASURE_Factory> FACTORY_CLASS= TILDA__MEASURE_Factory.class;
   public transient static final String TABLENAME = TextUtil.print("TILDA.Measure", "");
...
*/
   public void setSchema(String v) throws Exception
     {
       long T0 = System.nanoTime();
       if (v == null)
        throw new Exception("Cannot set tilda.data.TILDA.Measure.schema to null: it's not nullable.");
       else if (v.length() > 64)
        throw new Exception("Cannot set tilda.data.TILDA.Measure.schema: the size "+v.length()+" is larger than the max allowed of 64: "+TextUtil.escapeDoubleQuoteWithSlash(TextUtil.toMaxLength(v, 250)));
   ...
   public void setName(String v) throws Exception
     {
       ...
       if (v == null)
        ...
        throw new Exception("Cannot set tilda.data.TILDA.Measure.name: the size "+v.length()+" is larger than the max allowed of 64: "+TextUtil.escapeDoubleQuoteWithSlash(TextUtil.toMaxLength(v, 250)));
   ...
   public final boolean write(Connection C) throws Exception
     {
       ...
       if (__Init == null && __LookupId==0) // Loaded via some other mechamism, e.g., Json or CSV loader
        {
          ...
          initForCreate();
          // Auto PK
          setRefnum(tilda.db.KeysManager.getKey("TILDA.MEASURE"));
        }
       ...
       try
        ...
        {
          tilda.data._Tilda.TILDA__2_3.handleFinally(PS, T0, TILDA__MEASURE_Factory.SCHEMA_TABLENAME_LABEL, __Init == InitMode.CREATE ? StatementType.INSERT : StatementType.UPDATE, count, null);
          PS = null;
        }
       ...
       if (__Init == InitMode.CREATE)
        {
          ...
          int Pos = S.length();
          if (__Changes.intersects(TILDA__MEASURE_Factory.COLS.REFNUM._Mask) == true) { TILDA__MEASURE_Factory.COLS.REFNUM.getFullColumnVarForInsert(C, S); V.append(tilda.data._Tilda.TILDA__2_3._COMMAQUESTION);  }
          if (__Changes.intersects(TILDA__MEASURE_Factory.COLS.SCHEMA._Mask) == true) { TILDA__MEASURE_Factory.COLS.SCHEMA.getFullColumnVarForInsert(C, S); V.append(tilda.data._Tilda.TILDA__2_3._COMMAQUESTION);  }
          if (__Changes.intersects(TILDA__MEASURE_Factory.COLS.NAME._Mask) == true) { TILDA__MEASURE_Factory.COLS.NAME.getFullColumnVarForInsert(C, S); V.append(tilda.data._Tilda.TILDA__2_3._COMMAQUESTION);  }
          ...
          if (__Changes.intersects(TILDA__MEASURE_Factory.COLS.CREATED._Mask) == true) { TILDA__MEASURE_Factory.COLS.CREATED.getFullColumnVarForInsert(C, S);
             V.append(__Nulls.intersects(TILDA__MEASURE_Factory.COLS.CREATED._Mask) == false && DateTimeUtil.isNowPlaceholder(_created) == true ? C.getCommaCurrentTimestamp() : tilda.data._Tilda.TILDA__2_3._COMMAQUESTION);
           }
          ...
          if (__Changes.intersects(TILDA__MEASURE_Factory.COLS.LASTUPDATED._Mask) == true) { TILDA__MEASURE_Factory.COLS.LASTUPDATED.getFullColumnVarForInsert(C, S);
             V.append(__Nulls.intersects(TILDA__MEASURE_Factory.COLS.LASTUPDATED._Mask) == false && DateTimeUtil.isNowPlaceholder(_lastUpdated) == true ? C.getCommaCurrentTimestamp() : tilda.data._Tilda.TILDA__2_3._COMMAQUESTION);
           }
          ...
          if (__Changes.intersects(TILDA__MEASURE_Factory.COLS.DELETED._Mask) == true) { TILDA__MEASURE_Factory.COLS.DELETED.getFullColumnVarForInsert(C, S);
             V.append(__Nulls.intersects(TILDA__MEASURE_Factory.COLS.DELETED._Mask) == false && DateTimeUtil.isNowPlaceholder(_deleted) == true ? C.getCommaCurrentTimestamp() : tilda.data._Tilda.TILDA__2_3._COMMAQUESTION);
           }
          ...
          if (write(C) == false)
           {
             initForLookup(lookupId);
              // Undo auto PK
              __Changes.andNot(TILDA__MEASURE_Factory.COLS.REFNUM._Mask);
             return write(C);
           }
        }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   private final boolean readOne(Connection C, boolean Force) throws Exception
     {
       ...
       try
        ...
        {
          tilda.data._Tilda.TILDA__2_3.handleFinally(PS, T0, TILDA__MEASURE_Factory.SCHEMA_TABLENAME_LABEL, StatementType.SELECT, count, null);
          PS = null;
        }
    }
   ...
   boolean init(Connection C, java.sql.ResultSet RS) throws Exception
    {
      int i = 0;
     __Init = InitMode.LOOKUP;
      __Saved_refnum      = _refnum      =                              RS.getLong      (++i) ;  if (RS.wasNull() == true) { __Nulls.or(TILDA__MEASURE_Factory.COLS.REFNUM._Mask     ); _refnum = null; }
      __Saved_schema      = _schema      = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) { __Nulls.or(TILDA__MEASURE_Factory.COLS.SCHEMA._Mask     ); _schema = null; }
      __Saved_name        = _name        = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) { __Nulls.or(TILDA__MEASURE_Factory.COLS.NAME._Mask       ); _name = null; }
                            _created     = DateTimeUtil.toZonedDateTime(RS.getTimestamp(++i, DateTimeUtil._UTC_CALENDAR), null); if (RS.wasNull() == true) { __Nulls.or(TILDA__MEASURE_Factory.COLS.CREATED._Mask    ); _created = null; }
                            _lastUpdated = DateTimeUtil.toZonedDateTime(RS.getTimestamp(++i, DateTimeUtil._UTC_CALENDAR), null); if (RS.wasNull() == true) { __Nulls.or(TILDA__MEASURE_Factory.COLS.LASTUPDATED._Mask); _lastUpdated = null; }
                            _deleted     = DateTimeUtil.toZonedDateTime(RS.getTimestamp(++i, DateTimeUtil._UTC_CALENDAR), null); if (RS.wasNull() == true) { __Nulls.or(TILDA__MEASURE_Factory.COLS.DELETED._Mask    ); _deleted = null; }
     __LookupId = 0;
   ...
   public void toJSON(java.io.Writer out, String exportName, String lead, boolean fullObject, boolean noNullArrays) throws Exception
    {
      switch (exportName)
        { 
          case "": tilda.data.Measure_Factory.toJSON(out, (tilda.data.Measure_Data) this, lead, fullObject, noNullArrays); break;
          default: throw new Exception("Unknown JSON exporter '"+exportName+"' for tilda.data.Measure_Factory");
        } 
    }
   public void toJSON(java.io.Writer out, String exportName, String lead, boolean fullObject, java.time.ZonedDateTime lastsync) throws Exception
    {
      throw new Exception("Unknown JSON sync exporter '"+exportName+"' for tilda.data.Measure_Factory");
    }
   public void toCSV(java.io.Writer out, String exportName) throws Exception
    {
      switch (exportName)
        { 
          case "": tilda.data.Measure_Factory.toCSV(out, (tilda.data.Measure_Data) this); break;
          default: throw new Exception("Unknown CSV exporter '"+exportName+"' for tilda.data.Measure_Factory");
        } 
    }
   public void toCSV(java.io.Writer out, String exportName, java.time.ZonedDateTime lastsync) throws Exception
    {
      throw new Exception("Unknown CSV sync exporter '"+exportName+"' for tilda.data.Measure_Factory");
    }
 }
