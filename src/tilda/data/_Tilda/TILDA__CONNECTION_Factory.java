...
*/
   ...
   public static int WriteBatch(Connection C, List<tilda.data.Connection_Data> L, int logOffset) throws Exception
     {
       long T0 = System.nanoTime();
       ...
       try
         {
           ...
           for (tilda.data.Connection_Data d : L)
             {
               ...
               if (((TILDA__CONNECTION) d).__Init != InitMode.CREATE)
                 {
                   LOG.debug(QueryDetails._LOGGING_HEADER + "The 'tilda.data.Connection_Data' object at positon #" + indexOffset + " was not in an insertable state. Only inserts are allowed in batch writes (i.e., no updates).");
                   QueryDetails.setLastQuery(TILDA__CONNECTION_Factory.SCHEMA_TABLENAME_LABEL, "");
                   return indexOffset;
                 }
               ...
               if (((TILDA__CONNECTION) d).BeforeWrite(C) == false)
                 {
                   LOG.debug(QueryDetails._LOGGING_HEADER + "The 'tilda.data.Connection_Data' object at positon #" + indexOffset + " failed in its BeforeWrite() method.");
                   QueryDetails.setLastQuery(TILDA__CONNECTION_Factory.SCHEMA_TABLENAME_LABEL, "");
                   return indexOffset;
                 }
               ...
               if (firstChangeList.equals(((TILDA__CONNECTION) d).__Changes) == false)
                 {
                   LOG.debug(QueryDetails._LOGGING_HEADER + "The 'tilda.data.Connection_Data' object at positon #" + indexOffset + " failed matching the list of columns being changed compared to the first object passed.");
                   QueryDetails.setLastQuery(TILDA__CONNECTION_Factory.SCHEMA_TABLENAME_LABEL, "");
                   return indexOffset;
                 }
               ...
               if (firstTimeStampSignature.equals(((TILDA__CONNECTION) d).getTimeStampSignature()) == false)
                 {
                   LOG.debug(QueryDetails._LOGGING_HEADER + "The 'tilda.data.Connection_Data' object at positon #" + indexOffset + " failed matching the list of updated current vs value based timestamps.");
                   QueryDetails.setLastQuery(TILDA__CONNECTION_Factory.SCHEMA_TABLENAME_LABEL, "");
                   return indexOffset;
                 }
           ...
           if (JDBCHelper.BatchWriteDone(results, L.size() - insertCount) == false)
             {
               LOG.debug(QueryDetails._LOGGING_HEADER + "A batch of 'Connection_Data' objects ending at position #" + indexOffset + " failed being written to the database.");
               return indexOffset;
             }
