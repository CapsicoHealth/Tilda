   ...
   public void setSchemaName(String v) throws Exception
     {
       ...
       if (v == null)
        ...
        throw new Exception("Cannot set tilda.data.TILDA.MaintenanceLog.schemaName: the size "+v.length()+" is larger than the max allowed of 64: "+TextUtil.escapeDoubleQuoteWithSlash(TextUtil.toMaxLength(v, 250)));
   ...
   public void setStatement(String v) throws Exception
     {
       ...
       else if (v.length() > 256000)
        throw new Exception("Cannot set tilda.data.TILDA.MaintenanceLog.statement: the size "+v.length()+" is larger than the max allowed of 256000: "+TextUtil.escapeDoubleQuoteWithSlash(TextUtil.toMaxLength(v, 250)));
   ...
   boolean init(Connection C, java.sql.ResultSet RS) throws Exception
    {
     ...
     __Init = InitMode.LOOKUP;
      __Saved_refnum      = _refnum      =                              RS.getLong      (++i) ;  if (RS.wasNull() == true) { __Nulls.or(TILDA__MAINTENANCELOG_Factory.COLS.REFNUM._Mask     ); _refnum = null; }
                            ...
                            _deleted     = DateTimeUtil.toZonedDateTime(RS.getTimestamp(++i, DateTimeUtil._UTC_CALENDAR), null); if (RS.wasNull() == true) { __Nulls.or(TILDA__MAINTENANCELOG_Factory.COLS.DELETED._Mask    ); _deleted = null; }
     __LookupId = 0;
