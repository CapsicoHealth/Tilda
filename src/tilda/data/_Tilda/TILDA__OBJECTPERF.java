   ...
   protected int populatePreparedStatement(Connection C, java.sql.PreparedStatement PS, List<java.sql.Array> AllocatedArrays) throws Exception
     {
       ...
       if (__Changes.intersects(TILDA__OBJECTPERF_Factory.COLS.STARTPERIOD._Mask) == true) 
        { 
          if (__Nulls.intersects(TILDA__OBJECTPERF_Factory.COLS.STARTPERIOD._Mask) == true) PS.setNull(++i, java.sql.Types.TIMESTAMP_WITH_TIMEZONE); else if (DateTimeUtil.isNowPlaceholder(_startPeriod) == false) { LOG.debug("startPeriod: "+_startPeriod+" - "+java.sql.Timestamp.from(_startPeriod.toInstant())); PS.setTimestamp(++i, java.sql.Timestamp.from(_startPeriod.toInstant()), DateTimeUtil._UTC_CALENDAR); }
        } 
       ...
       if (__Changes.intersects(TILDA__OBJECTPERF_Factory.COLS.ENDPERIOD._Mask) == true) 
        { 
          if (__Nulls.intersects(TILDA__OBJECTPERF_Factory.COLS.ENDPERIOD._Mask) == true) PS.setNull(++i, java.sql.Types.TIMESTAMP_WITH_TIMEZONE); else if (DateTimeUtil.isNowPlaceholder(_endPeriod) == false) { LOG.debug("endPeriod: "+_endPeriod+" - "+java.sql.Timestamp.from(_endPeriod.toInstant())); PS.setTimestamp(++i, java.sql.Timestamp.from(_endPeriod.toInstant()), DateTimeUtil._UTC_CALENDAR); }
        } 
   ...
   boolean init(Connection C, java.sql.ResultSet RS) throws Exception
    {
     ...
     __Init = InitMode.LOOKUP;
      ...
      __Saved_startPeriod   = _startPeriod   = JDBCHelper.processZDT(_startPeriodTZ  , "tilda.data.TILDA.ObjectPerf.startPeriod"  , RS, ++i, TILDA__OBJECTPERF_Factory.COLS.STARTPERIOD  , TILDA__OBJECTPERF_Factory.COLS.STARTPERIODTZ  , __Nulls); if (RS.wasNull() == true) { __Nulls.or(TILDA__OBJECTPERF_Factory.COLS.STARTPERIOD._Mask  ); _startPeriod = null; }
                              ...
                              _endPeriod     = JDBCHelper.processZDT(_endPeriodTZ    , "tilda.data.TILDA.ObjectPerf.endPeriod"    , RS, ++i, TILDA__OBJECTPERF_Factory.COLS.ENDPERIOD    , TILDA__OBJECTPERF_Factory.COLS.ENDPERIODTZ    , __Nulls); if (RS.wasNull() == true) { __Nulls.or(TILDA__OBJECTPERF_Factory.COLS.ENDPERIOD._Mask    ); _endPeriod = null; }
