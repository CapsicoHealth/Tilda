...
*/
   ...
   public static int WriteBatch(Connection C, List<tilda.data.TransPerf_Data> L, int logOffset) throws Exception
     {
       long T0 = System.nanoTime();
       ...
       try
         {
           ...
           for (tilda.data.TransPerf_Data d : L)
             {
               ...
               if (((TILDA__TRANSPERF) d).__Init != InitMode.CREATE)
                 {
                   LOG.debug(QueryDetails._LOGGING_HEADER + "The 'tilda.data.TransPerf_Data' object at positon #" + indexOffset + " was not in an insertable state. Only inserts are allowed in batch writes (i.e., no updates).");
                   QueryDetails.setLastQuery(TILDA__TRANSPERF_Factory.SCHEMA_TABLENAME_LABEL, "");
                   return indexOffset;
                 }
               ...
               if (((TILDA__TRANSPERF) d).BeforeWrite(C) == false)
                 {
                   LOG.debug(QueryDetails._LOGGING_HEADER + "The 'tilda.data.TransPerf_Data' object at positon #" + indexOffset + " failed in its BeforeWrite() method.");
                   QueryDetails.setLastQuery(TILDA__TRANSPERF_Factory.SCHEMA_TABLENAME_LABEL, "");
                   return indexOffset;
                 }
               ...
               if (firstChangeList.equals(((TILDA__TRANSPERF) d).__Changes) == false)
                 {
                   LOG.debug(QueryDetails._LOGGING_HEADER + "The 'tilda.data.TransPerf_Data' object at positon #" + indexOffset + " failed matching the list of columns being changed compared to the first object passed.");
                   QueryDetails.setLastQuery(TILDA__TRANSPERF_Factory.SCHEMA_TABLENAME_LABEL, "");
                   return indexOffset;
                 }
               ...
               if (firstTimeStampSignature.equals(((TILDA__TRANSPERF) d).getTimeStampSignature()) == false)
                 {
                   LOG.debug(QueryDetails._LOGGING_HEADER + "The 'tilda.data.TransPerf_Data' object at positon #" + indexOffset + " failed matching the list of updated current vs value based timestamps.");
                   QueryDetails.setLastQuery(TILDA__TRANSPERF_Factory.SCHEMA_TABLENAME_LABEL, "");
                   return indexOffset;
                 }
           ...
           if (JDBCHelper.BatchWriteDone(results, L.size() - insertCount) == false)
             {
               LOG.debug(QueryDetails._LOGGING_HEADER + "A batch of 'TransPerf_Data' objects ending at position #" + indexOffset + " failed being written to the database.");
               return indexOffset;
             }
