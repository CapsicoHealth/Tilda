...
<TD align="center">&#x2610&nbsp;&nbsp;</TD>
<TD align="center">-&nbsp;&nbsp;</TD>
<TD>Message details<DIV style="margin:0px;margin-left:20px;font-size:75%;">&rarr;&nbsp;<A href="TILDA___Docs.TILDA.html#JobPartMessage-msg_DIV">TILDA<B>&nbsp;&#8226;&nbsp;</B>JobPartMessage<B>&nbsp;&#8226;&nbsp;</B>msg</A></DIV></TD>
</TR>
</TABLE></BLOCKQUOTE>
</DIV>
...
*/
   ...
   public String  Str_jobDataStart;
   ZonedDateTime _jobDataStart = null;
...
*/
   ...
   public String  Str_jobDataEnd;
   ZonedDateTime _jobDataEnd = null;
...
*/
   ...
   public String  Str_jobStart;
   ZonedDateTime _jobStart = null;
...
*/
   ...
   public String  Str_jobEnd;
   ZonedDateTime _jobEnd = null;
...
*/
   ...
   public String  Str_jobPartDataStart;
   ZonedDateTime _jobPartDataStart = null;
...
*/
   ...
   public String  Str_jobPartDataEnd;
   ZonedDateTime _jobPartDataEnd = null;
...
*/
   ...
   public String  Str_jobPartStart;
   ZonedDateTime _jobPartStart = null;
...
*/
   ...
   public String  Str_jobPartEnd;
   ZonedDateTime _jobPartEnd = null;
   ...
   boolean init(Connection C, java.sql.ResultSet RS) throws Exception
    {
      int i = 0;
     __Init = InitMode.LOOKUP;
                                    _jobRefnum           =                              RS.getLong      (++i) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBREFNUM._Mask          );
                                    _jobName             = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBNAME._Mask            );
                                    _jobType             = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBTYPE._Mask            );
                                    _jobUserId           = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBUSERID._Mask          );
                                    _jobDataStartTZ      = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBDATASTARTTZ._Mask     ); else _jobDataStartTZ      = _jobDataStartTZ     .trim();
                                    _jobDataStart        = processZDT(_jobDataStartTZ       , "tilda.data.TILDA.JobView.jobDataStart"       , RS, ++i, TILDA__JOBVIEW_Factory.COLS.JOBDATASTART       , TILDA__JOBVIEW_Factory.COLS.JOBDATASTARTTZ       ); if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBDATASTART._Mask       );
                                    _jobDataEndTZ        = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBDATAENDTZ._Mask       ); else _jobDataEndTZ        = _jobDataEndTZ       .trim();
                                    _jobDataEnd          = processZDT(_jobDataEndTZ         , "tilda.data.TILDA.JobView.jobDataEnd"         , RS, ++i, TILDA__JOBVIEW_Factory.COLS.JOBDATAEND         , TILDA__JOBVIEW_Factory.COLS.JOBDATAENDTZ         ); if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBDATAEND._Mask         );
                                    _jobStartTZ          = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBSTARTTZ._Mask         ); else _jobStartTZ          = _jobStartTZ         .trim();
                                    _jobStart            = processZDT(_jobStartTZ           , "tilda.data.TILDA.JobView.jobStart"           , RS, ++i, TILDA__JOBVIEW_Factory.COLS.JOBSTART           , TILDA__JOBVIEW_Factory.COLS.JOBSTARTTZ           ); if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBSTART._Mask           );
                                    _jobEndTZ            = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBENDTZ._Mask           ); else _jobEndTZ            = _jobEndTZ           .trim();
                                    _jobEnd              = processZDT(_jobEndTZ             , "tilda.data.TILDA.JobView.jobEnd"             , RS, ++i, TILDA__JOBVIEW_Factory.COLS.JOBEND             , TILDA__JOBVIEW_Factory.COLS.JOBENDTZ             ); if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBEND._Mask             );
                                    _jobStatus           =                              RS.getBoolean   (++i) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBSTATUS._Mask          );
                                    _jobMsg              = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBMSG._Mask             );
                                    _jobPartName         = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTNAME._Mask        );
                                    _jobPartType         = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTTYPE._Mask        );
                                    _jobPartDataStartTZ  = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTDATASTARTTZ._Mask ); else _jobPartDataStartTZ  = _jobPartDataStartTZ .trim();
                                    _jobPartDataStart    = processZDT(_jobPartDataStartTZ   , "tilda.data.TILDA.JobView.jobPartDataStart"   , RS, ++i, TILDA__JOBVIEW_Factory.COLS.JOBPARTDATASTART   , TILDA__JOBVIEW_Factory.COLS.JOBPARTDATASTARTTZ   ); if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTDATASTART._Mask   );
                                    _jobPartDataEndTZ    = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTDATAENDTZ._Mask   ); else _jobPartDataEndTZ    = _jobPartDataEndTZ   .trim();
                                    _jobPartDataEnd      = processZDT(_jobPartDataEndTZ     , "tilda.data.TILDA.JobView.jobPartDataEnd"     , RS, ++i, TILDA__JOBVIEW_Factory.COLS.JOBPARTDATAEND     , TILDA__JOBVIEW_Factory.COLS.JOBPARTDATAENDTZ     ); if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTDATAEND._Mask     );
                                    _jobPartStartTZ      = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTSTARTTZ._Mask     ); else _jobPartStartTZ      = _jobPartStartTZ     .trim();
                                    _jobPartStart        = processZDT(_jobPartStartTZ       , "tilda.data.TILDA.JobView.jobPartStart"       , RS, ++i, TILDA__JOBVIEW_Factory.COLS.JOBPARTSTART       , TILDA__JOBVIEW_Factory.COLS.JOBPARTSTARTTZ       ); if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTSTART._Mask       );
                                    _jobPartEndTZ        = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTENDTZ._Mask       ); else _jobPartEndTZ        = _jobPartEndTZ       .trim();
                                    _jobPartEnd          = processZDT(_jobPartEndTZ         , "tilda.data.TILDA.JobView.jobPartEnd"         , RS, ++i, TILDA__JOBVIEW_Factory.COLS.JOBPARTEND         , TILDA__JOBVIEW_Factory.COLS.JOBPARTENDTZ         ); if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTEND._Mask         );
                                    _jobPartRecordsCount =                              RS.getInt       (++i) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTRECORDSCOUNT._Mask);
                                    _jobPartStatus       =                              RS.getBoolean   (++i) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTSTATUS._Mask      );
                                    _jobPartNotify       =                              RS.getBoolean   (++i) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTNOTIFY._Mask      );
                                    _jobPartMessage      = TextUtil.trim               (RS.getString    (++i)) ;  if (RS.wasNull() == true) __Nulls.or(TILDA__JOBVIEW_Factory.COLS.JOBPARTMESSAGE._Mask     );
     __LookupId = 0;
